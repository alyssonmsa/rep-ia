...
Stack: **Svelte 5 + Vite + TypeScript**, chosen for the lightest possible bundle (compiles away the framework runtime), matching the §4 requirement of a fast first load on bad 3G. Fonts are self-hosted via `@fontsource/*` packages (no runtime font fetches). Build is currently on **Fatia 1** of the §10 build order (stage mode with hardcoded fixture songs — no IndexedDB, no editing, no import yet); later slices will add persistence, the text-format parser, PWA/offline installability, and the three themes as described in the sections below.
